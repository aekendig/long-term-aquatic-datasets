...
# loop through taxa
pdf("output/nitrogen_time_series_by_taxon.pdf")
...
for(i in inv_taxa){
  ...
  print(ggplot(subdat, aes(x = GSYear, y = QualityMean, color = PermanentID)) +
          ...
          labs(x = "Year", y = "total nitrogen (ug/L)", title = i) +
          def_theme_paper +
...
# save data
write_csv(dat, "intermediate-data/FWC_nitrogen_analysis_formatted.csv")
...
ggplot(dat, aes(x = RecentTreatment, y = LogQualityMean)) +
  ...
  # geom_smooth(method = "lm") +
  facet_wrap(~ CommonName, scales = "free")
# N increases the more recently lake has been treated for floating plants
...
# hydrilla treatment increases N until location is accounted for, then decrease
# hydrilla slightly reduces N
# recent treatment has a weaker effect than treated
# interaction is minimal

# accounting for location reduces the effects of floating plants and treatment on N
# floating decreases N and treatment increases it
# recent treatment has a stronger effect
# negative effect of plants only with treatment

# accounting for location reduces negative treatment effect of Cuban bulrush
# recent treatment is stronger than treated
# treatment and plants both reduce N
# interaction is minimal

# accounting for location switches the effects of torpedograss to positive
# plant increases N and treatment reduces it
# recent similar to treated
# interaction reduces N even more
...
# export models
save(hydr_mean_mod, file = "output/fwc_hydrilla_mean_nitrogen_model.rda")
save(flpl_mean_mod, file = "output/fwc_floating_plant_mean_nitrogen_model.rda")
save(cubu_mean_mod, file = "output/fwc_cuban_bulrush_mean_nitrogen_model.rda")
save(torp_mean_mod, file = "output/fwc_torpedograss_mean_nitrogen_model.rda")

save(hydr_max_mod, file = "output/fwc_hydrilla_max_nitrogen_model.rda")
save(flpl_max_mod, file = "output/fwc_floating_plant_max_nitrogen_model.rda")
save(cubu_max_mod, file = "output/fwc_cuban_bulrush_max_nitrogen_model.rda")
save(torp_max_mod, file = "output/fwc_torpedograss_max_nitrogen_model.rda")

save(hydr_min_mod, file = "output/fwc_hydrilla_min_nitrogen_model.rda")
save(flpl_min_mod, file = "output/fwc_floating_plant_min_nitrogen_model.rda")
save(cubu_min_mod, file = "output/fwc_cuban_bulrush_min_nitrogen_model.rda")
save(torp_min_mod, file = "output/fwc_torpedograss_min_nitrogen_model.rda")

save(hydr_cv_mod, file = "output/fwc_hydrilla_cv_nitrogen_model.rda")
save(flpl_cv_mod, file = "output/fwc_floating_plant_cv_nitrogen_model.rda")
save(cubu_cv_mod, file = "output/fwc_cuban_bulrush_cv_nitrogen_model.rda")
