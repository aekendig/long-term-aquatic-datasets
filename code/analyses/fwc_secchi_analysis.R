...
uninv_sum <- uninv %>%
  ...
  group_by(CommonName) %>%
  summarize(UninvMean = mean(QualityMean),
            UninvMax = mean(QualityMax),
            UninvMin = mean(QualityMin),
            UninvCV = mean(QualityCV),
            UninvN = n()) %>%
  ungroup() %>%
  pivot_longer(cols = c(UninvMean, UninvMax, UninvMin, UninvCV),
               names_to = "Response",
               values_to = "Uninv",
               names_prefix = "Uninv") %>%
...
#### start here ####
# creating table for BAA results
# trim down for manuscript

# identify significant effects
foc_sig <- foc_mod_se %>%
  left_join(mod_coef_fun(list(hydr_mean_mod, hydr_max_mod, hydr_min_mod, hydr_cv_mod), "hydrilla") %>%
