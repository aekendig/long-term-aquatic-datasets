...
ctrl_lag_fun <- function(dat){
  ...
  outdat <- dat %>%
    ...
    summarise(PropTreated = sum(PropTreated),
              AreaTreated_ha = sum(AreaTreated_ha)) %>% # add proportion lake treated for all treatments in a year (can exceed 1)
    ungroup() %>%
    ...
    summarise(PropTreated = mean(PropTreated), # average proportion treated per year 
              ...
              Treated = as.numeric(PropTreated > 0)) %>% # average area treated per year
    ungroup()
