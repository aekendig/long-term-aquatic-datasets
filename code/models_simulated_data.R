...
# import data
dat_global <- read_csv("intermediate-data/simulated_data_pre_herbicide_global_model.csv")
...
dat_global2 <- filter(dat_global, !is.na(logNDiff))

# data
input_global1 <- list(TT = max(dat_global2$time),
                     y = dat_global2$logNDiff,
                     x = dat_global2$N)
...
# summary
sink("output/global_process_model_1.txt")
print(fit_global1)
sink()
