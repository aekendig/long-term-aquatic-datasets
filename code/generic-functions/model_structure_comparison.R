...
mod_structure_comp <- function(simp_mods, ran_mods, fix_mods) {
  ...
  for(i in 1:length(simp_mods)){
    ...
    simp_sum[[i]] <- simp_sum[[i]] %>%
      mutate(coefficients = names(coef(simp_mods[[i]]))) # add coefficient names
  ...
  for(i in 1:length(ran_mods)){
    ...
    ran_sum[[i]] <- ran_sum[[i]] %>%
      mutate(coefficients = names(fixef(ran_mods[[i]])$cond)) # add coefficient names
  ...
  for(i in 1:length(fix_mods)){
    ...
    fix_sum[[i]] <- fix_sum[[i]] %>%
      mutate(coefficients = names(coef(fix_mods[[i]]))) # add coefficient names
